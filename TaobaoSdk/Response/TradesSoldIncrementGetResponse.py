...
## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">Response: 1. 搜索当前会话用户作为卖家已卖出的增量交易数据 2. 只能查询时间跨度为一天的增量交易记录 3. 返回数据结果为创建订单时间的倒序 4. 只能查询3个月内修改过的数据，超过这个时间的数据无法通过taobao.trade.fullinfo.get获取详情。</SPAN>
# <UL>
# <LI>
